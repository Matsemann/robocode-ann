...
* [Bug-337][]: Hangups with New Editor in 1.7.4.0.
	* Notice, that the editor can still be quite slow with large source files. Work is still in progress to velocity it up.
...
* Upgraded to Maven 2.2.1 used for building Robocode.
	* Cleaned up the build, improved the velocity, and got rid of issue with building and testing every twice.
...
* Fixed three bugs introduced with the Beta version found using new test units:
	* One bug where the velocity would immediately drop to max. velocity when calling ``setMaxVelocity()`` - disregarding the acceleration and deceleration rules.
...
#### New TPS Slider
* The TPS slider has been redesigned to be more exponential, so it covers battle in slow velocity (1-30 TPS), higher velocity (30 - 120 TPS), and fast velocity (120 - 1000 TPS).
...
* [Bug-113][]: ``-battle`` option runs at full velocity.
	* When using the ``-battle`` option Robocode would run at full velocity, i.e. the TPS set to maximum, even though the GUI was enabled with a predefined TPS.
...
* Lots of internal optimizations of Robocode has been made to velocity up the game.
...
### Bug Fixes
* [Bug-16][]: Too many skipped turn because of CPU velocity detection.
	* Robots than ran fine under v1.0.6 were skipping turns like crazy under v1.0.7.
	* The CPU velocity detection has been changed to accept 50 times as many clock cycles than with with v1.0.7.
...
* New system for calculating CPU velocity.
...
* Robocode will now always run at full velocity when minimized.
...
* Battles/Rounds start faster (back to 0.96.2 velocity).
...
[Bug-16]:  http://sourceforge.net/p/robocode/bugs/16/   (Too many skipped turn because of CPU velocity detection)
...
[Bug-113]: http://sourceforge.net/p/robocode/bugs/113/  (-battle option runs at full velocity)
